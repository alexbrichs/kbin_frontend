    ...
    <blockquote :class="'section comment entry-comment subject comment-level--1'"
      ...
      <header>
        ...
        <time class="timeago" :datetime="comment.creation_data">&nbsp;{{
          ...
          <a :href="`/thread/${extractThreadId(comment.parent)}/top`" >{{ titol_parent }}</a>
        </time>
...
export default {
  ...
  methods: {
    async fetchParent() {
      try {
        const response = await axios.get(`https://bravo13-36a68ba47d34.herokuapp.com/api/publicacions/${this.extractThreadId(this.comment.parent)}/`)
        this.titol_parent = response.data.title
      } catch (error) {
